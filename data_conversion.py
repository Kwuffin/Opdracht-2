...
cnx = psycopg2.connect("dbname=huwebshop user=postgres password=pass")  # TODO: Edit dbname, user, and password accordingly.
cursor = cnx.cursor()
...
def get_values(normalized, collection, values, get_fk):
    """
    Gets the values ready to write into csv
    :param normalized:
    :param collection:
    :param values:
    :param get_fk:
...
def get_path(file_name):
    """
    Gets the path of the saved .csv file
    :param file_name:
    :return:
    """
...
def create_csv_file(table_name):
    """
    Writes the csv file to the local machine
    :param table_name:
    :return:
    """
...
def upload_files():
    """
    Imports .csv files into the PostgreSQL database
    :return:
    """
    file_names = ["brand", "category", "sub_category", "sub_sub_category", "color", "gender",
                  "products", "profiles", "sessions", "products_bought"]
    problem_files = [9, 10]
...
def create_tables():
    """
    Defines all the values for tables
    :return:
    """
